...
public class EventsData
{
    ...
    public List<DbDurationEvent> CreateManyDurationEvents(int count, Guid userId, DateOnly start, DateOnly end,
        Guid? eventTypeId = null)
    {
        if (end < start)
    ...
    public List<DbOccurrenceEvent> CreateManyOccurrenceEvents(int count, Guid userId, DateOnly start, DateOnly end,
        Guid? eventTypeId = null)
    {
        if (end < start)
            throw new ArgumentException($"{nameof(end)}({end}) must be greater or equal to {nameof(start)}({start})",
